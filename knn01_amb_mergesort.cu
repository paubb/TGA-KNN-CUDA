...
//FUNCTIONS MERGE SORT FROM ANOTHER PROJECT
...
int classifyAPointCUDA(Point arr[], float val[], int n, int k, Point p)
{
    ...
    float *arrSorted_h_val, *arrSortedF_h_val;
    float *arrSorted_d_val, *arrSortedF_d_val;
    ...
    nThreads_sort = 128;
    nBlocks_sort = 32;
    chunkSize_sort = n/(nThreads*nBlocks);
    ...
    if (PINNED) {
        ...
        cudaMallocHost((float**)&arrSorted_h_val, nBytes_sort);
        cudaMallocHost((float**)&arrSortedF_h_val, nBytes_sort);
        ...
        arrSorted_h_val = (float*) malloc(nBytes_sort);
        arrSortedF_h_val = (float*) malloc(nBytes_sort);
    ...
    cudaMalloc((float**)&arrSorted_d_val, nBytes_sort);
    cudaMalloc((float**)&arrSortedF_d_val, nBytes_sort);
    ...
    // Sort the Points by distance from p
    callMergeSort<<<nBlocks, nThreads>>>(result_prediction_dev, arrSorted_d, ref_points_dev_val, arrSorted_d_val, chunkSize_sort, n);
    int auxChunkSize = chunkSize_sort*2;
    int auxBlock = nBlocks_sort;
    int auxThread = nThreads_sort/2;
    while (auxChunkSize < N) {
       callMerge<<<auxBlock, auxThread>>>(arrSorted_d, arrSortedF_d, arrSorted_d_val, arrSortedF_d_val, auxChunkSize, n);
       auxChunkSize = auxChunkSize*2;
       auxThread = auxThread/2;
    }
    //cudaMemcpy(arrSorted_h, arrSortedF_d, nBytes_sort, cudaMemcpyDeviceToHost);
    //cudaMemcpy(arrSorted_h_val, arrSortedF_d_val, nBytes_sort, cudaMemcpyDeviceToHost);
    ...
    */
    ...
    // Ejecutar el kernel
    calculateFreq<<<k, 1>>>(arrSortedF_d_val, freq_dev);
